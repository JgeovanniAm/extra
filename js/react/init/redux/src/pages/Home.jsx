...
export default () => {
    ...
    const [collectDeaths, setCollectDeaths] = useState(null);
    // main data of my API
    ...
    let [numCharacter, setNumCharacter] = useState(6);
    useEffect(() => {
    ...
    /**
     * 
     * @param target is the element that I have clicked
     */

    function sortByName({ target }) {
        let resultSortName;
        setView(target.id);
        if (target.id === 'portrayer') {
            resultSortName = data.sort((a, b) =>
